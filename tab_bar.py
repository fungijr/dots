...
#BLOCK_ICON = ""
#blank icons for colored squares
CALENDAR_ICON = ""
TERMINAL_ICON = ""
ERR_ICON = ""
REFRESH_TIME = 1
...
def _get_active_process_name_cell() -> dict:
    cell = {"icon": TERMINAL_ICON, "icon_bg_color": "#FF8D80", "text": ""}
    boss = get_boss()
...
def _get_date_cell() -> dict:
    now = datetime.now().strftime("%m-%d-%y")
    return {"icon": CALENDAR_ICON, "icon_bg_color": "#A351D6", "text" : now}

    return cell


def _create_cells() -> list[dict]:
    return [_get_active_process_name_cell(), _get_date_cell()]
...
def _draw_right_status(screen: Screen, is_last: bool, draw_data: DrawData) -> int:
    ...
    tab_fg = as_rgb(int(to_color("#B5B5B5")))
    ...
    for c in cells:
        ...
        padding = 1
        screen.draw(" " * padding)
        screen.cursor.bg = as_rgb(int(to_color("#212121")))
